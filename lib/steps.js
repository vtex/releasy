...
var steps = {
  ...
  scripts: (msg, config, key) => {
    const pkg = /package\.json$/.test(config.versionProvider.filePath[0])
    const manifest = /manifest\.json$/.test(config.versionProvider.filePath[1])
  ...
  add: config => {
    ...
    let successMessage = `File(s) ${config.versionProvider.filePath.join(' ')} added`
    if (fs.existsSync(config.changelogPath)) {
      gitAddCommand += ` ${config.changelogPath}`
      successMessage = `Files ${config.versionProvider.filePath.join(' ')} ${config.changelogPath} added`
    }
    return steps.run(gitAddCommand, successMessage, config.dryRun, config.quiet)
  },
  commit: config => {
    let successMessage = `File(s) ${config.versionProvider.filePath.join(' ')} commited`
    if (fs.existsSync(config.changelogPath)) {
      successMessage =
        `Files ${config.versionProvider.filePath.join(' ')} ${config.changelogPath} commited`
    }
